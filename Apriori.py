...
class Apriori:
    ...
    def create_database(self, min_supp):
        ...
        print(f'The size of the database is {self.num_rows} rows, so with a min_support of {min_supp}, '
              f'an itemset must exist in at least {min_rows} rows to be above threshold.\n')
        ...
        # when creating itemsets (ex: for column name = Complaint Type and cell = "Noise", the cell gets converted
        # to "Complaint Type = Noise". This is to disambiguate items in item baskets.
        for a in attributes:
            c = a[1]
            columns.append(c)
    ...
    def generate_assocation_rules(self):
        ...
        while self.get_C_size() > 0:
            ...
            # for each itemset (row) in the table, generate all possible association rules with one item on the RHS
            for itemset in itemsets:
                ...
                for l in LHS:
                    for r in itemset:
                        if r not in l:
                            # retrieve the confidence of the rule and save it if it is above threshold
    ...
    # generates example-run.txt by printing each frequent itemset and association rule that is above threshold
    def create_output_file(self):
        ...
        # sort the dictionary of itemsets by their support in descending order and print them/write them
        ...
        # sort rules by their confidence in descending order and print them/write them
...
def main():
    if len(sys.argv) != 4:
        print('Invalid number of input arguments.\nFormat: Apriori.py '
              'INTEGRATED_DATASET.CSV <min_supp> <min_conf>')
        exit(1)
    ...
    A = Apriori(min_supp=min_support, min_conf=min_confidence)
    A.apriori()
